...
in terms of a logarithmic scale, just as we did with word count ratios. If something is half as common in Barack Obama's, that should be as far from "100%" as something that is twice as common.
...
bigrams %>% select(word, word2, pmi) %>% head(10)
```
...
might occur *randomly.* It bears a close relationship to log-likelihood. In addition to calculating the observed-expected ratio 
for a word occurring in a corpus, you also calculate the observed-expected ratio for the word in all the works *not* in that corpus.
...
counts <- allSOTUs %>%
  ...
  # Count the occurrences of each bigram
  count(word, word2) %>%
  group_by(word) %>% summarize_pmi(word2, n, all_fields = TRUE)

pmid <- counts %>%
  summarize_pmi(word, n, all_fields = TRUE) %>% 
  inner_join(counts)
...
counts %>% 
  ...
  mutate(
    ...
    signed_dunning = dunning * ifelse(.pmi < 0, -1, 1 )) -> dun_test

dun_test %>% arrange(pmi2)
...
authorProbs <- fedWords %>%
  # We're using only Mosteller's list.
...
authorProbs %>% filter(author %in% c("HAMILTON", "MADISON")) %>% group_by(author) %>% summarize_llr(word, n) %>% arrange(-abs(dunning_llr))
...
fedMatrix = authorProbs %>%
  spread(word, probability, fill = 0) %>% ungroup %>% 
...
model = glm(author == "HAMILTON" ~ there + by + on, data = known, family = "binomial")
model %>% summary
...
model = glm(author == "HAMILTON" ~ there + by, data = train, family = "binomial")
predictions = test %>%
  mutate(.prediction = predict(model, newdata = .)) %>%
